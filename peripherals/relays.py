...
def actuate_time_hod(output, time_on = 0, time_off = 0, interval = 1, interval_units = "minutes", wattage = "0", log = None): #updates every 20 minutes by default 
    ...
    if time_on < time_off:
        if HoD >= time_on and HoD < time_off:
            ...
            if log is not None:
                cs.write_state("/home/pi/oasis-cpu/configs/power_data.json", log, str(float(cs.structs["power_data"][log])+ physics.kwh(float(wattage),float(time_active))))
...
def actuate_interval_sleep(output, duration = 15, sleep = 45, duration_units = "seconds", sleep_units = "seconds", wattage = "0", log = None):
    ...
    if log is not None:
            cs.write_state("/home/pi/oasis-cpu/configs/power_data.json", log, str(physics.kwh(float(wattage),float(time_active))))
