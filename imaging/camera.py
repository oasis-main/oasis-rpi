...
def save_to_feed(image_path):
    ...
    save_most_recent = rusty_pipes.Open(["cp", str(image_path), "/home/pi/oasis-cpu/data_out/image_feed/image_at_" + str(timestamp)+'.jpg'],"cp")
...
def actuate(interval: int, nosleep = False): #interval is amount of time between shots in minutes, nosleep skips the wait
    ...
    exit_status = take_picture('/home/pi/oasis-cpu/data_out/image.jpg')
    
    if exit_status == 0:

        #Load, resize, and save image:
        image = noir_ndvi.cv2.imread('/home/pi/oasis-cpu/data_out/image.jpg')
        ...
        noir_ndvi.cv2.imwrite('/home/pi/oasis-cpu/data_out/image.jpg' ,resized_image)

        #Get NDVI if active
        if cs.structs["feature_toggles"]["ndvi"] == "1":
            noir_ndvi.convert_image('/home/pi/oasis-cpu/data_out/image.jpg')

        #Save to feed if active
        if cs.structs["feature_toggles"]["save_images"] == "1":
            save_to_feed('/home/pi/oasis-cpu/data_out/image.jpg')

        #Sent to cloud if connected
        if cs.structs["device_state"]["connected"] == "1":
            #send new image to firebase
            send_image('/home/pi/oasis-cpu/data_out/image.jpg', image_filename="image.jpg")
    ...
    try:    
        while True:
            ...
            if (time.time() - float(cs.structs["hardware_config"]["camera_settings"]["last_picture_time"])) > (float(cs.structs["hardware_config"]["camera_settings"]["picture_frequency"])*60): #convert setting (minutes) to base units (seconds)
                cs.write_nested_state("/home/pi/oasis-cpu/configs/hardware_config.json", "camera_settings" ,"last_picture_time", str(time.time()), db_writer = dbt.patch_firebase) #we MUST ALWAYS write before sleeping, otherwise the program will double-count the wait period!
