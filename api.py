...
def set_temperature_target(value):
    cs.load_state()
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "target_temperature", str(value))
    print("Temperature target was set to: " + str(value) + " degrees farenheit.")
    return

def set_humidity_target(value):
    cs.load_state()
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "target_humidity", str(value))
...
def set_light_timer(time_start_light, time_stop_light, lighting_interval):
    cs.load_state()
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "time_start_light", str(time_start_light))
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "time_stop_light", str(time_stop_light))
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "lighting_interval", str(lighting_interval))
...
def set_camera_interval(value):
    cs.load_state()
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "camera_interval", str(value))
    print("Camera will take a picture every " + str(value) + " seconds.")
    return

def set_watering_cycle(watering_duration, watering_interval):
    cs.load_state()
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "watering_duration", str(watering_duration))
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "watering_interval", str(watering_interval))
...
def set_air_timer(time_start_air,time_stop_air, air_interval):
    cs.load_state()
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "", str(time_start_air))
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "", str(time_stop_air))
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "", str(air_interval))
...
def set_heater_response(value):
    cs.load_state()
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "P_temp", str(value))
    print("Heater response gain set to: " + str(value))
    return

def set_heater_damping(value):
    cs.load_state()
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "D_temp", str(value))
    print("Heater damping gain set to: " + str(value))
    return

def set_humidifier_response(value):
    cs.load_state()
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "P_hum", str(value))
    print("Humidifier response gain set to: " + str(value))
    return

def set_humidifier_damping(value):
    cs.load_state()
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "D_hum", str(value))
    print("Humidifier damping gain set to: " + str(value))
    return

def set_fan_response_temp(value):
    cs.load_state()
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "Pt_fan", str(value))
    print("Fan temperature response gain set to: " + str(value))
    return    

def set_fan_damping_temp(value):
    cs.load_state()
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "Dt_fan", str(value))
    print("Fan temperature damping gain set to: " + str(value))
    return

def set_fan_response_hum(value):
    cs.load_state()
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "Ph_fan", str(value))
    print("Fan humidity response gain set to: " + str(value))
    return

def set_fan_damping_temp(value):
    cs.load_state()
    cs.write_state("/home/pi/oasis-grow/configs/device_params.json", "Dh_fan", str(value))
    print("Fan humidity response gain set to: " + str(value))
    return
